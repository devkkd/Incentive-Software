    ...
    try {
      ...
      if (!res.ok) {
        ...
        setOtpError(remaining > 0
          ? `Incorrect code. ${remaining} attempt${remaining !== 1 ? 's' : ''} remaining before this code is cancelled.`
        ...
        {step === 1 && (
          <>
            ...
            <p className="text-[13px] text-gray-600 mb-6 leading-relaxed">
              Enter the email address linked to your account. We'll send you a one-time code to verify it's you.
            </p>
            ...
            <form onSubmit={handleSendOtp} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-gray-400">Enter your email linked to your account.</p>
              </div>
            ...
            <div className="mt-6 text-center">
              <Link href="/" className="text-[13px] font-bold text-[#2B3B8A] hover:underline">← Back to Sign In</Link>
            </div>
          </>
        )}
        ...
        {step === 2 && (
          <>
            ...
            <div className="flex gap-2 mb-2">
              {otp.map((digit, index) => (
                <input key={index} ref={(el) => (otpInputRefs.current[index] = el)}
                  ...
                  className={`w-full aspect-square max-w-[52px] text-center text-lg font-medium rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2B3B8A] bg-white placeholder:text-gray-300 border transition-colors ${
                    otpInvalid ? 'border-red-400 bg-red-50' : 'border-gray-200'
                  }`}
                />
              ))}
            </div>
            ...
            <button onClick={handleVerifyOtp} disabled={loading || otp.join('').length < 6 || attemptsLeft === 0}
              className={`w-[80%] mx-auto block font-semibold py-3 rounded-xl transition-colors flex items-center justify-center gap-2 ${
                otp.join('').length === 6 && !loading && attemptsLeft > 0
                  ? 'bg-[#2B3B8A] hover:bg-[#1f2b66] text-white'
            ...
            <form onSubmit={handleSetPassword} className="space-y-4">
              ...
              <div className="bg-[#F4F7FB] border border-[#E2E8F0] rounded-xl p-3 flex gap-2 items-start">
                ...
                <p className="text-[12px] text-gray-700 leading-relaxed">
                  <span className="font-bold text-gray-900">Password must:</span><br />
