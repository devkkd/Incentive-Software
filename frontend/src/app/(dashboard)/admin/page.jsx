...
export default function AdminDashboardPage() {
  ...
  const kpi2Items = [
    ...
    { title: 'Total Vendors/Party', value: loading ? '...' : kpi2.totalVendors?.toLocaleString() || '0', icon: <path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" /> },
    { title: 'Total FTC All Division', value: loading ? '...' : kpi2.totalDivisions?.toLocaleString() || '0', icon: <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z" /> },
          ...
          {pieData.length > 0 ? (
            <div className="flex-1 flex items-center justify-between mt-4 relative">
              <div className="w-[180px] h-[180px]">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={pieData} innerRadius={45} outerRadius={85} paddingAngle={2} dataKey="value" stroke="none">
                ...
                {pieData.map((item, index) => (
                  <div key={index} className="flex items-start gap-2">
                    ...
                    <div className="text-[11px] text-gray-800 font-medium leading-tight">
                      ...
                      <p className="text-gray-500">{fmt(item.amount)}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex-1 flex items-center justify-center text-gray-400 text-[13px]">No division data</div>
          )}

          <div className="text-center mt-4">
            <h3 className="text-xl font-bold text-black">{fmt(stats?.totalDivisionAmount)}</h3>
          </div>
        </div>
      </div>
