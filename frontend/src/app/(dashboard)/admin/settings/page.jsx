...
export default function AdminSettingsPage() {
  ...
  const handleSendOtp = async () => {
    if (!oldPassword || !newPassword || !confirmPassword) { setPassError('Pehle saare password fields fill karo'); return; }
    if (newPassword !== confirmPassword) { setPassError('New password aur confirm password match nahi kar rahe'); return; }
    if (newPassword.length < 8) { setPassError('Password kam se kam 8 characters ka hona chahiye'); return; }

    setSendingOtp(true); setPassError('');
    try {
      const res = await fetch(`${API}/api/settings/password/send-otp`, { method: 'POST', headers: authHeaders(), credentials: 'include' });
      const data = await res.json();
      if (!res.ok) { setPassError(data.message); return; }
      setOtpEmail(data.email); setOtpSent(true); setCountdown(45); setOtp(['', '', '', '', '', '']);
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      setOtpSent(false); setOldPassword(''); setNewPassword(''); setConfirmPassword(''); setOtp(['', '', '', '', '', '']);
    } catch { setPassError('Server error'); }
    ...
    <div className="p-8 md:p-10 max-w-[1600px] mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col lg:flex-row overflow-hidden min-h-[500px]">
        ...
        <div className="w-full lg:w-1/3 p-8 md:p-10 border-b lg:border-b-0 lg:border-r border-gray-100 flex flex-col">
          ...
          {passError && (
            <div className="mb-4 p-3 bg-[#FDEDEC] border border-[#E74C3C]/20 rounded-xl text-[13px] text-red-700 font-medium">{passError}</div>
          )}
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input type={showConfirm ? 'text' : 'password'} value={confirmPassword}
                  ...
                  className={`w-full px-4 py-3 rounded-xl border text-sm focus:outline-none focus:ring-1 focus:ring-[#2B3B8A] transition-colors ${confirmPassword && newPassword !== confirmPassword ? 'border-[#E74C3C]' : 'border-gray-200'}`} />
                <EyeIcon show={showConfirm} toggle={() => setShowConfirm(!showConfirm)} />
              </div>
              {confirmPassword && newPassword !== confirmPassword && <p className="text-[12px] text-[#E74C3C]">Passwords do not match</p>}
            </div>
          ...
          <button onClick={handleSendOtp} disabled={sendingOtp || otpSent}
            className={`mt-6 self-start font-semibold px-6 py-3 rounded-xl flex items-center gap-2 transition-colors ${sendingOtp || otpSent ? 'bg-[#8492A6] text-white cursor-not-allowed' : 'bg-[#2B3B8A] hover:bg-[#1a2d6b] text-white'}`}>
        ...
        <div className="w-full lg:w-1/3 p-8 md:p-10 flex flex-col">
          ...
          {!otpSent ? (
            ...
            <div className="space-y-4 flex-1">
              ...
              <button onClick={handleChangePassword} disabled={otp.join('').length < 6 || changingPass}
                className={`w-full font-semibold py-3 rounded-xl flex items-center justify-center gap-2 transition-colors ${otp.join('').length === 6 && !changingPass ? 'bg-[#2B3B8A] hover:bg-[#1a2d6b] text-white' : 'bg-[#CBD5E1] text-[#64748B] cursor-not-allowed'}`}>
                {changingPass ? 'Changing Password...' : 'Verify & Change Password →'}
              </button>
              <div className="text-[12px] text-gray-700">
                <p>Didn't receive the code?</p>
                {countdown > 0 ? (
                  <p className="text-[#2B3B8A] font-semibold mt-0.5">Resend available in <span className="text-gray-500 font-normal">{countdown}s</span></p>
                ) : (
                  <button onClick={() => { setOtpSent(false); handleSendOtp(); }} className="text-[#2B3B8A] font-semibold mt-0.5 hover:underline">Resend code now</button>
                )}
              </div>
            </div>
          )}
