  ...
  try {
    ...
    if (!companyName || !accountNumber || !mobileNumber) {
      return res
        ...
        .json({
          success: false,
          message: "Party Name, Party Code and Mobile Number are required",
        });
    }
    ...
    if (existing) {
      return res.status(409).json({
        success: false,
        message:
          existing.accountNumber === prefixedAccountNumber
    ...
    try {
      ...
      const normalize = (obj) => {
        ...
        for (const key of Object.keys(obj))
          result[key.toLowerCase().replace(/\s+/g, " ").trim()] = obj[key];
        return result;
      };
      ...
      for (const rawRow of rows) {
        ...
        const loc = String(row["location"] || row["loc"] || "")
          .trim()
          .toUpperCase();
        const consPartyCode = String(
          row["party code"] ||
            row["cons party code"] ||
            row["cons_party_code"] ||
            "",
        ).trim();
        const consPartyName = String(
          row["party name"] ||
            row["cons party name"] ||
            row["cons_party_name"] ||
            "",
        ...
        const city = String(
          row["party city"] ||
            row["cons party city desc"] ||
            row["cons_party_city_desc"] ||
            row["city"] ||
            "",
        ).trim();
        const partyType = String(
          row["party type"] || row["party_type"] || "",
        ).trim();
        const mobileNumber = String(
          row["mobile no"] || row["mobile_no"] || row["mobile"] || "",
        ).trim();
        const salesPerson = String(
          row["sales person name"] ||
            row["sales person"] ||
            row["sales_person"] ||
            "",
        ).trim();
        ...
        if (!loc || !consPartyCode || !consPartyName) {
          results.failed.push({
            row: consPartyCode || "N/A",
            reason:
              "Missing required fields: Loc, Cons Party Code, Cons Party Name",
          });
          continue;
        }
        ...
        if (!division) {
          results.failed.push({
            row: consPartyCode,
            reason: `Division not found for location code: ${loc}`,
          });
          continue;
        }
...
router.get("/", protect, async (req, res) => {
  try {
    ...
    if (isBranch) {
      ...
      const masterWallets = await Wallet.find({ _id: { $in: walletIds } })
        .select("_id isHold")
        .lean();
      const masterWalletMap = {};
      masterWallets.forEach((w) => {
        masterWalletMap[String(w._id)] = w.isHold;
      });
      ...
      monthlyWallets.forEach((mw) => {
        ...
        usableMap[vid] = parseFloat(
          ((usableMap[vid] || 0) + mw.balance).toFixed(2),
        );
      });
  ...
  try {
    ...
    if (divisionId || accountNumber) {
      ...
      if (!suffix) {
        return res
          .status(400)
          .json({ success: false, message: "Party code cannot be empty" });
      }
      ...
      if (existing) {
        return res
          ...
          .json({
            success: false,
            message:
              existing.accountNumber === updatedAccountNumber
                ? "This account number already exists"
                : "This mobile number is already registered",
          });
      }
    ...
    vendor.companyName =
      companyName !== undefined ? companyName : vendor.companyName;
    vendor.personName =
      personName !== undefined ? personName : vendor.personName;
    vendor.mobileNumber =
      mobileNumber !== undefined ? mobileNumber : vendor.mobileNumber;
    ...
    vendor.salesPerson =
      salesPerson !== undefined ? salesPerson : vendor.salesPerson;
    ...
    const [rawTransactions, vendor] = await Promise.all([
      WalletTransaction.find({ vendor: req.params.id })
        ...
        .populate({
          path: "invoice",
          select:
            "invoiceNumber referenceNo invoiceDate invoiceAmount location remark division",
    ...
    try {
      ...
      for (const vendorId of vendorIds) {
        const vendor = await Vendor.findById(vendorId)
          .select("companyName walletBalance")
          .lean();
        if (!vendor) continue;
        ...
        const currentBalance = parseFloat(
          (vendor.walletBalance || 0).toFixed(2),
        );
    ...
    try {
      ...
      if (existingDebits.length > 0) {
        return res.status(400).json({
          success: false,
          message:
            "Debit transactions already exist for some of these invoices",
          existing: existingDebits.map((t) => ({
            invoice: t.invoice,
            amount: t.amount,
          })),
        });
