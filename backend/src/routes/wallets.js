...
router.get("/", protect, async (req, res) => {
  try {
    ...
    syncLegacyWallets().catch((err) =>
      console.error("[syncLegacyWallets]", err.message),
    );
    ...
    const totalCreditedFromTxn = parseFloat(
      (creditAgg[0]?.total || 0).toFixed(2),
    );
    ...
    const trueSystemBalance = parseFloat(
      (totalCreditedFromTxn - totalRedeemed).toFixed(2),
    );
    ...
    const result = await Promise.all(
      wallets.map(async (w) => {
        ...
        const vendorDocs = await Vendor.find({
          _id: { $in: partyWallets.map((p) => p.vendor) },
        })
          .select("_id status")
          .lean();
        const vendorMap = new Map(
          vendorDocs.map((v) => [String(v._id), v.status]),
        );

        const live = partyWallets.filter(
          (pw) => pw.vendor && vendorMap.has(String(pw.vendor)),
        );
        const orphan = partyWallets.filter(
          (pw) => !pw.vendor || !vendorMap.has(String(pw.vendor)),
        );

        const sum = (arr, f) =>
          parseFloat(arr.reduce((a, c) => a + (c[f] || 0), 0).toFixed(2));
        ...
        const heldBalance = sum(
          live.filter((pw) => w.isHold || pw.isHold),
          "balance",
        );
        const freeBalance = sum(
          live.filter((pw) => !w.isHold && !pw.isHold),
          "balance",
        );
        ...
        const negativeBalance = sum(
          live.filter((pw) => (pw.balance || 0) < 0),
          "balance",
        );
    ...
    const t = (f) =>
      parseFloat(result.reduce((a, w) => a + (w[f] || 0), 0).toFixed(2));
    ...
    const cardTotalRedeemed = parseFloat(
      (cardTotalCredited - cardTotalBalance).toFixed(2),
    );

    // System Balance = Credited - Redeemed  (all money still on the books)
    const cardSystemBalance = parseFloat(
      (cardTotalCredited - cardTotalRedeemed).toFixed(2),
    );
    ...
    const cardOrphanCount = result.reduce(
      (a, w) => a + (w.orphanCount || 0),
      0,
    );
    const cardNegativeBalance = t("negativeBalance");
    const cardNegativeCount = result.reduce(
      (a, w) => a + (w.negativeCount || 0),
      0,
    );
...
router.post("/", protect, authorize("branch", "admin"), async (req, res) => {
  try {
    ...
    if (!name || !name.trim()) {
      return res
        .status(400)
        .json({ success: false, message: "Wallet name is required" });
    }
  ...
  try {
    ...
    const monthlyWallets = await MonthlyWallet.find(filter)
      .populate({
        path: "vendor",
        select:
          "companyName personName accountNumber mobileNumber partyCity partyType status walletBalance division",
    ...
    try {
      ...
      if (typeof isHold !== "boolean") {
        return res
          ...
          .json({
            success: false,
            message: "isHold boolean field is required",
          });
      }
      ...
      wallet.holdReason = isHold
        ? holdReason || "Wallet placed on hold by admin"
        : null;
      await wallet.save();
      ...
      res.status(200).json({
        success: true,
        message: isHold
          ? `Wallet "${wallet.name}" is now on hold`
          : `Wallet "${wallet.name}" released from hold`,
        data: wallet,
      });
    } catch (error) {
      res.status(500).json({ success: false, message: error.message });
  ...
  async (req, res) => {
    try {
      ...
      if (!monthlyWalletId) {
        return res
          .status(400)
          .json({ success: false, message: "monthlyWalletId is required" });
      }
      if (typeof isHold !== "boolean") {
        return res
          .status(400)
      ...
      const mw = await MonthlyWallet.findById(monthlyWalletId).populate(
        "vendor",
        "companyName",
      );
      if (!mw) {
        return res
          .status(404)
          .json({ success: false, message: "Party wallet record not found" });
      }

      mw.isHold = isHold;
      mw.holdReason = isHold
        ? holdReason || "Party balance placed on hold"
        : null;
      await mw.save();
...
router.get("/diagnostics", protect, authorize("admin"), async (req, res) => {
  try {
    ...
    const orphanDebitTotal = parseFloat(
      (orphanDebits[0]?.total || 0).toFixed(2),
    );
    const orphanDebitCount = orphanDebits[0]?.count || 0;

    const newFlowDebitTotal = parseFloat(
      (totalDebits - orphanDebitTotal).toFixed(2),
    );
    const adjustedExpectedBalance = parseFloat(
      (totalCredits - newFlowDebitTotal).toFixed(2),
    );
    ...
    res.status(200).json({
      ...
      data: {
        ...
        wouldReconcileAfterCleanup:
          Math.abs(vendorBalanceSum - adjustedExpectedBalance) < 1,
        discrepancy: parseFloat(
          (vendorBalanceSum - expectedBalance).toFixed(2),
        ),
        discrepancyAfterCleanup: parseFloat(
          (vendorBalanceSum - adjustedExpectedBalance).toFixed(2),
        ),
      },
    });
  } catch (error) {
